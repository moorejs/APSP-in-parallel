...
void johnson_parallel(graph_t *gr, int** output) {
  ...
  gr->edge_array = new Edge[bf_graph->E];
  gr->weights = new int[bf_graph->E];
  ...
  std::vector<int> d(bf_graph->V, (std::numeric_limits < short >::max)());
  ...
  bool r = bellman_ford_shortest_paths(BFG, bf_graph->V, distance_map(&d[0]));
  if (!r) {
  ...
  for (int e = 0; e < gr->E; e++) {
    ...
    gr->weights[e] = gr->weights[e] + d[u] - d[v];
  }
